...
- **A symmetric war map (M0).** The nests sit at equal offsets either side of the world centre and every generator — food clusters, moisture, rocky regions, berry bushes — runs once per colony around its own nest, rotated by π, so the terrain is one empire's reflected through the middle. Contested larders and every carcass land on the midline and are smellable by both. Soldiers hold a picket posted toward the enemy instead of orbiting their own queen, walk home when hungry, and steer toward the strongest enemy trail in range.
...
- **The war is one battle, not a campaign.** M0 landed the antagonist: over five seeds at 40k ticks the rival now wins one run in five (it won none before), median combat deaths are 24 player / 20 rival (they were 0 / 1), and rival deliveries roughly doubled. But contact concentrates in the first two sim-minutes and then the front stabilises into a standoff. Recurring raids and war tides are S1-B and unbuilt.
...
3. **Green before merge:** runtime-affecting changes run `npm test`, `npm run typecheck`, and `npm run build`. Documentation-only commits, including agent instructions, skills, and workflow metadata, use focused validation when the change includes no application code, simulation code, tests, build configuration, application dependencies, scenarios, or generated-source inputs. The scope check must account for staged, unstaged, and untracked files.
...
6. **Forbidden:** fleet reports, findings docs, plan-goal bundles, progress ledgers, statistical validation sweeps, baseline re-anchoring, calendar estimates, and any new document other than `DEMO.md`, updates to this GDD's inventory section when the ground truth changes, and domain documentation created under the Agent Skills rules (`CONTEXT.md`, `CONTEXT-MAP.md`, and ADRs under `docs/adr/` or context-scoped `src/*/docs/adr/`). Domain documentation is created lazily only when terms or decisions are actually resolved, never as speculative scaffolding.
